...
function canRenderAccessForm(item: PlatformAdminAccess, current: PlatformAdminAccess) {
  return item.source === "database" && canManagePlatformAccess(current) && !isPrimaryPlatformOwnerEmail(item.email);
}
...
export default async function PlatformAdminPage({ searchParams }: { searchParams?: Promise<PlatformAdminSearchParams | undefined> }) {
  ...
  const canManageAccess = canManagePlatformAccess(access);
  ...
  return (
    <div className="space-y-5">
      ...
      <section id="suscripciones" className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card className="p-3">
          ...
          <p className="mt-3 rounded-xl bg-[var(--app-surface-muted)] p-3 text-xs leading-5 text-[var(--app-text-muted)]">
            Para agregar otros correos, usa la sección Accesos admin globales. Solo OWNER puede hacerlo.
          </p>
        </Card>
      </section>
      ...
      <Card id="accesos" className="p-3">
        ...
        {canManageAccess ? (
          <form action={addPlatformAdminAccessAction} className="mb-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_150px_minmax(0,1fr)_auto]">
          ...
          {adminAccesses.map((item) => (
            <CompactCard key={item.email} className="space-y-2">
              ...
              {canRenderAccessForm(item, access) ? (
                ...
                <p className="text-xs font-bold text-[var(--app-text-muted)]">{isPrimaryPlatformOwnerEmail(item.email) ? "El dueño principal no se puede eliminar ni desactivar." : "Solo OWNER puede editar accesos."}</p>
              )}
            </CompactCard>
          ))}
        </div>
