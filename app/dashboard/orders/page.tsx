    ...
    <div>
      ...
      <div className="grid gap-5 xl:grid-cols-[380px_1fr]">
        <Card>
          ...
          <p className="mt-1 text-sm text-[var(--app-text-muted)]">Solo convierte cotizaciones que el cliente ya aceptó para evitar crear pedidos innecesarios.</p>
          <form action={createOrderFromQuoteAction} className="mt-5 space-y-3">
            <label className="block text-sm font-semibold text-[var(--app-text)]">
              Cotización aceptada
              <span className="mt-1 block text-xs text-[var(--app-text-muted)]">Selecciona la cotización que quieres transformar en pedido.</span>
            ...
            <button className="w-full rounded-2xl bg-[var(--app-primary)] px-4 py-3 font-bold text-[var(--app-button-text)]">Crear pedido y descontar stock</button>
          </form>
          {acceptedQuotes.length === 0 && <p className="mt-4 text-sm text-[var(--app-text-muted)]">No hay cotizaciones aceptadas pendientes. Una vez aceptada una cotización podrás crear el pedido aquí.</p>}
        </Card>
            ...
            orders.map((order) => (
              <Card key={order.id}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--app-text-muted)]">#{order.id.slice(-6).toUpperCase()}</p>
                    <h2 className="mt-1 text-xl font-black">{order.customer?.name ?? order.customer?.phone ?? "Cliente sin datos"}</h2>
                    <p className="mt-1 text-sm text-[var(--app-text-muted)]">{order.items.length} productos · {order.createdAt.toLocaleDateString("es-CL")}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-black">{formatCLP(order.total)}</p>
                    <p className="text-sm text-[var(--app-text-muted)]">Estado {order.status}</p>
                  </div>
                </div>
                <div className="mt-4 grid gap-2">
                  {order.items.map((item) => (
                    <div key={item.id} className="flex justify-between rounded-2xl bg-[var(--app-surface-muted)] px-4 py-3 text-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <Link href={`/dashboard/orders/${order.id}`} className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] px-4 py-2 text-sm font-bold">Ver detalle</Link>
                </div>
              </Card>
            ))
          )}
