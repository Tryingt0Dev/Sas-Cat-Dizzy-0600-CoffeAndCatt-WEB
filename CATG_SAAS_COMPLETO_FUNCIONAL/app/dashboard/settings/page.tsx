...
import { allowedTemplatesForPlan, getBusinessPlan } from "@/services/plan-guard";
...
export default async function SettingsPage({ searchParams }: { searchParams?: Promise<SettingsSearchParams | undefined> }) {
  ...
  const currentTemplateAllowed = allowedTemplates.includes(business.catalogTemplate as (typeof allowedTemplates)[number]);
            ...
            >
              <div className="grid gap-3 md:grid-cols-2">
                {catalogTemplateOptions.map((template) => {
                  const allowed = allowedTemplates.includes(template.value);
                  return (
                    <label
                      key={template.value}
                      className={allowed
                        ? "cursor-pointer rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 transition hover:bg-[var(--app-surface)]"
                        : "rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 opacity-65"}
                    >
                      <div className="flex items-start gap-3">
                        <input
                          ...
                          defaultChecked={allowed && business.catalogTemplate === template.value}
                          disabled={!allowed}
                          className="mt-1"
                        />
                        <span className="min-w-0">
                          <span className="flex flex-wrap items-center gap-2 text-sm font-black">
                            {template.label}
                            {!allowed ? <StatusBadge variant="warning" className="px-2 py-0.5 text-[0.65rem]">Plan superior</StatusBadge> : null}
                          </span>
                          <span className="mt-1 block text-xs leading-5 text-[var(--app-text-muted)]">{template.description}</span>
                        </span>
                      </div>
                    </label>
                  );
                })}
                {!currentTemplateAllowed ? <input type="hidden" name="catalogTemplate" value={business.catalogTemplate} /> : null}
              </div>
          ...
          <section id="avanzado" className="scroll-mt-20">
            ...
            >
              <div className="grid gap-4">
                ...
                {canUseAdvancedBranding ? (
                  <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3">
                    <h3 className="text-sm font-black">Branding del catálogo</h3>
                    <LiveThemeColorControls
                      initialPrimary={business.primaryColor ?? "#111827"}
                      initialSecondary={business.secondaryColor ?? "#F9FAFB"}
                      initialAccent={business.accentColor ?? "#E11D48"}
                      initialBackground={business.backgroundColor ?? "#F8FAFC"}
                  ...
                  <>
                    ...
                    <LockedFeature title="Branding avanzado" description="Colores, radio visual, logo y banner personalizados requieren un plan superior." />
                  </>
                )}
