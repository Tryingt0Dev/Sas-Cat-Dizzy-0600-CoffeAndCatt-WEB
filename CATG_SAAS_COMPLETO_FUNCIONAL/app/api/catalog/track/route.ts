...
export async function POST(req: Request) {
  try {
    // SECURITY: Verify origin for mutating catalog tracking endpoint
    ...
    const business = await prisma.business.findFirst({
      where: { publicSlug: payload.data.businessSlug, isActive: true },
      select: { id: true }
    });
    ...
    try {
      await assertRateLimit(rateLimitKey({ endpoint: "catalog:track", businessId: business.id, ip }), 180, 15 * 60 * 1000);
    } catch (error) {
