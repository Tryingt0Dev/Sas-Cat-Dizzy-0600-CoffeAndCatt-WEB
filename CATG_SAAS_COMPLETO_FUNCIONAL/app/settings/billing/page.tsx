...
function usageLine(label: string, used: number, max: PlanLimitValue) {
  return (
    <div className="rounded-2xl bg-[var(--app-surface-muted)] p-4">
      <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--app-text-muted)]">{label}</p>
      <p className="mt-2 text-2xl font-black text-[var(--app-text)]">{used} / {formatPlanLimit(max)}</p>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_380px]">
          <Card>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl bg-[var(--app-surface-muted)] p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--app-text-muted)]">Plan actual</p>
                <h2 className="mt-2 text-3xl font-black text-[var(--app-text)]">{currentPlanDefinition.name}</h2>
                <p className="mt-2 text-sm leading-6 text-[var(--app-text-muted)]">{currentPlanDefinition.description}</p>
              </div>
              <div className="rounded-2xl bg-[var(--app-surface-muted)] p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--app-text-muted)]">Estado</p>
                <p className="mt-2 text-2xl font-black text-[var(--app-text)]">{subscriptionStatus}</p>
                <p className="mt-2 text-sm text-[var(--app-text-muted)]">No hay cobro automatizado activo desde esta pantalla.</p>
              </div>
              <div className="rounded-2xl bg-[var(--app-surface-muted)] p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--app-text-muted)]">Tienda</p>
                <p className="mt-2 text-2xl font-black text-[var(--app-text)]">{business.name}</p>
                <p className="mt-2 text-sm text-[var(--app-text-muted)]">/{business.publicSlug}</p>
              </div>
            </div>
          </Card>

          <Card>
            <h2 className="text-xl font-black text-[var(--app-text)]">Uso actual</h2>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {planList.map((plan) => {
            ...
            return (
              <Card key={plan.slug} className={isCurrent ? "border-[var(--app-primary)]" : undefined}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h2 className="text-2xl font-black text-[var(--app-text)]">{plan.name}</h2>
                    <p className="mt-2 text-sm leading-6 text-[var(--app-text-muted)]">{plan.description}</p>
                  </div>
                  {isCurrent ? <span className="rounded-full bg-[var(--app-primary)] px-3 py-1 text-xs font-black text-[var(--app-button-text)]">Actual</span> : null}
                </div>
                <div className="mt-5 grid gap-2 text-sm text-[var(--app-text-muted)]">
                  <p><strong className="text-[var(--app-text)]">{formatPlanLimit(entitlements.maxProducts)}</strong> productos</p>
                  <p><strong className="text-[var(--app-text)]">{formatPlanLimit(entitlements.maxUsersPerStore)}</strong> usuarios por tienda</p>
                  <p><strong className="text-[var(--app-text)]">{formatStorage(entitlements.maxStorageMb)}</strong> almacenamiento</p>
                  <p><strong className="text-[var(--app-text)]">{entitlements.aiAssistant}</strong> IA</p>
                </div>
                {isCurrent ? (
                  <button disabled className="mt-6 w-full rounded-2xl bg-[var(--app-surface-muted)] px-4 py-3 text-sm font-black text-[var(--app-text-muted)]">Plan actual</button>
                ) : (
                  <form action={requestPlanUpgradeAction} className="mt-6">
                    <input type="hidden" name="businessId" value={business.id} />
                    <input type="hidden" name="requestedPlan" value={plan.slug} />
                    <button className="w-full rounded-2xl bg-[var(--app-primary)] px-4 py-3 text-sm font-black text-[var(--app-button-text)] hover:bg-[var(--app-primary-hover)]">
        ...
        <Card>
          <h2 className="text-xl font-black text-[var(--app-text)]">Comparacion de capacidades</h2>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {comparisonRows.map((row) => (
                  <tr key={row.label} className="border-b border-[var(--app-border)]">
                    <td className="py-4 font-black text-[var(--app-text)]">{row.label}</td>
