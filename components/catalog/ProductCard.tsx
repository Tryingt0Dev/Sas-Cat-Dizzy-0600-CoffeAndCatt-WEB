  ...
  return (
    <article className="group flex h-full min-w-0 flex-col overflow-hidden rounded-[var(--catalog-radius)] border border-[var(--catalog-border)] bg-[var(--catalog-surface)] shadow-sm transition duration-200 hover:border-[var(--catalog-accent)] hover:shadow-md">
          ...
          {(product.discountPercent > 0 || product.featured) && (
            <div className="absolute top-1.5 left-1.5 flex flex-wrap gap-1">
              {product.discountPercent > 0 && (
                <span className="rounded-full bg-red-600 px-2 py-0.5 text-[0.65rem] font-black text-white">-{product.discountPercent}%</span>
              )}
              {product.featured && (
                <span className="rounded-full bg-[var(--catalog-accent)] px-2 py-0.5 text-[0.65rem] font-black text-[var(--catalog-accent-text)]">Destacado</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-2">
        <div>
          <p className="truncate text-[0.65rem] font-black uppercase tracking-[0.1em] text-[var(--catalog-text-muted)]">{product.category?.name ?? "Catalogo"}</p>
          <h3 className="mt-0.5 min-h-[2.5rem] text-sm font-bold leading-5 text-[var(--catalog-text)] line-clamp-2">
        ...
        <div className="mt-auto space-y-1.5">
          ...
          <div className="flex items-center justify-between gap-2 text-[0.65rem] font-bold">
            ...
            <Link
              href={productHref}
              className="rounded-[var(--catalog-radius)] bg-[var(--catalog-primary)] px-2.5 py-1.5 text-center text-xs font-black text-[var(--catalog-button-text)] transition duration-200 hover:bg-[var(--catalog-primary-hover)]"
            >
              Ver detalle
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <WhatsAppProductButton
              ...
              className="inline-flex min-h-8 items-center justify-center rounded-[var(--catalog-radius)] border border-[var(--catalog-border)] bg-[var(--catalog-surface)] px-1.5 py-1 text-center text-[0.65rem] font-black leading-tight text-[var(--catalog-text)] transition duration-200 hover:border-emerald-500 hover:text-emerald-600"
              disabledClassName="inline-flex min-h-8 items-center justify-center rounded-[var(--catalog-radius)] border border-[var(--catalog-border)] bg-[var(--catalog-muted)] px-1.5 py-1 text-center text-[0.65rem] font-black leading-tight text-[var(--catalog-text-muted)]"
            />
            <AskAiButton
              productId={product.id}
              productContext={productAiContext}
              question={buildProductAiQuestion(productAiContext)}
              className="inline-flex min-h-8 items-center justify-center rounded-[var(--catalog-radius)] border border-[var(--catalog-border)] bg-[var(--catalog-surface)] px-1.5 py-1 text-center text-[0.65rem] font-black leading-tight text-[var(--catalog-text)] transition duration-200 hover:border-[var(--catalog-accent)] hover:text-[var(--catalog-accent)]"
            />
          </div>
        </div>
      </div>
