...
## Estado de correcciones aplicadas desde la auditoría

Fecha de actualización: 2026-05-28

| Hallazgo | Estado actual | Archivo corregido | Fecha | Observación |
| -------- | ------------- | ----------------- | ----- | ----------- |
| Exportación CSV owner sin autenticación/autorización | Corregido en fase 1 | `app/api/admin/export-owner-csv/route.ts`, `lib/api-security.ts` | 2026-05-28 | Ahora exige platform-admin con permiso de billing, origen permitido, rate limit y valida rango de fechas. |
| Métricas históricas globales sin autenticación/autorización | Corregido en fase 1 | `app/api/admin/metrics-history/route.ts`, `lib/api-security.ts` | 2026-05-28 | Ahora exige platform-admin con permiso de billing y limita el rango `days` a 1-90. |
| Sincronización de métricas owner sin autenticación/autorización | Corregido en fase 1 | `app/api/admin/sync-metrics/route.ts`, `lib/api-security.ts` | 2026-05-28 | Ahora exige platform-admin con permiso de billing, origen permitido y rate limit. |
| Mutación/verificación de dominios custom por id sin tenant guard | Corregido en fase 1 | `app/api/stores/[id]/domains/route.ts` | 2026-05-28 | Ahora usa acceso tenant con `manage_settings`, valida origen, plan, dominio, rate limit y token criptográfico. |
| Catálogo público resuelve `customDomain` sin exigir verificación | Corregido en fase 1 | `app/store/[slug]/page.tsx` | 2026-05-28 | La resolución por host exige `customDomainVerified: true` en metadata y render. |
| `.env.example` apunta a SQLite aunque el schema activo usa PostgreSQL | Corregido en fase 1 | `.env.example` | 2026-05-28 | El ejemplo ahora usa PostgreSQL y mantiene placeholders sin secretos. |
| Helper `assertCanManageUser` puede devolver usuarios fuera del tenant | Corregido en fase 1 | `lib/security/tenant.ts` | 2026-05-28 | Ahora filtra por owner/membership del tenant salvo platform-admin. |
| Turnstile puede omitirse por configuración parcial | Corregido en fase 1 | `lib/turnstile.ts`, `app/(auth)/actions.ts`, `next.config.mjs` | 2026-05-28 | El registro exige Turnstile en producción o cuando hay keys parciales; CSP agrega Cloudflare Turnstile. |
| Fallo del proveedor de email no siempre detiene/traza verificación | Parcialmente corregido | `lib/email.ts`, `app/(auth)/actions.ts`, `app/api/auth/resend-verification/route.ts` | 2026-05-28 | Resend `!ok` ya se considera fallo; en producción el registro bloquea si no se puede enviar verificación. |
| Cookie de tienda seleccionada con `httpOnly: false` | Corregido en fase 1 | `app/(auth)/actions.ts` | 2026-05-28 | La cookie server-side se emite con `httpOnly: true`. |
| Middleware UI/API no fuerza redirects en `/api` | Corregido en fase 1 | `app/middleware.ts` | 2026-05-28 | El middleware ahora deja pasar `/api` al handler y evita redirecciones en APIs. |
| Token de dominio admin legacy inseguro | Corregido en fase 1 | `app/admin/actions.ts` | 2026-05-28 | El token de dominio se genera con `crypto.randomBytes` y la verificación valida `customDomain` antes de DNS. |
| Falta de tests negativos para APIs críticas | Parcialmente corregido | `scripts/pr08-security-audit-smoke.ts`, `package.json` | 2026-05-28 | Se agregó smoke test estático para guardias críticas; faltan pruebas HTTP/e2e reales. |
| Billing checkout/portal incompleto | Pendiente | `app/api/billing/checkout/route.ts`, `app/api/billing/portal/route.ts` | 2026-05-28 | No se corrigió porque requiere decisión de proveedor de pagos. |
| Webhook billing sin idempotencia real | Pendiente | `app/api/billing/webhook/route.ts` | 2026-05-28 | No se corrigió porque requiere contrato real de eventos y posible cambio de schema. |
| Rate limit en memoria para producción multi-instancia | Pendiente | `lib/rate-limit.ts`, `.env.example` | 2026-05-28 | Requiere decisión de infraestructura Redis/Upstash antes de hacerlo obligatorio. |
| Owner fallback hard-coded | Pendiente | `lib/platform-admin.ts` | 2026-05-28 | No se retiró para evitar bloquear acceso operativo sin decisión manual. |
| `schema_postgres.prisma` stale | Pendiente | `prisma/schema_postgres.prisma` | 2026-05-28 | Requiere decisión sobre scripts históricos `db:*:pg`. |
| Migración placeholder | Pendiente | `prisma/migrations/20260528031849_npx_prisma_validatenpx_prisma_migrate_status/migration.sql` | 2026-05-28 | No se modifica migración potencialmente aplicada. |
